...
$password =  $_POST['password'];
	...
	if(move_uploaded_file($tmp, $path)){ // Cek apakah gambar berhasil diupload atau tidak
		// Query untuk menampilkan data barbermna berdasarkan username yang dikirim
		$query = "SELECT * FROM data_barberman WHERE username_bm='".$username."'";
		$sql = mysqli_query($koneksi, $query); // Eksekusi/Jalankan query dari variabel $query
		$data = mysqli_fetch_array($sql); // Ambil data dari hasil eksekusi $sql

		// Cek apakah file foto sebelumnya ada di folder images
		if(is_file("../barberman/img/".$data['foto_bm'])) // Jika foto ada
			unlink("../barberman/img/".$data['foto_bm']); // Hapus file foto sebelumnya yang ada di folder images
		...
		$sql = mysqli_query($koneksi, $query); // Eksekusi/ Jalankan query dari variabel $query
			...
			// Jika Sukses, Lakukan :
			header("location: ubahprofil.php"); // Redirect ke halaman index.php
			}else{
			// Jika Gagal, Lakukan :
			echo "Maaf, Terjadi kesalahan saat mencoba untuk menyimpan data ke database.";
			echo "<br><a href='ubahprofil.php'>Kembali Ke Form</a>";
			}
	}else{
